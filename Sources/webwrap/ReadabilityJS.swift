...
// https://github.com/mozilla/readability — Apache License 2.0 (see the license
// header inside each source string; original copyright Arc90 Inc / Mozilla).
//
// Embedded as string constants because generated app bundles ship only the
// webwrap executable — a resource file would not travel into them. To upgrade:
// replace both literals with the files from the new tag and update the tag here
// and in the version constant below.

/// The Readability.js sources the host injects to power reader mode.
enum ReadabilityJS {
    /// Upstream tag the literals below were copied from (shown nowhere yet;
    /// kept so an upgrade is a one-line diff away from being traceable).
    static let version = "0.6.0"

    /// Readability.js — the article extractor (`new Readability(doc).parse()`).
...
"""#
    ...
    /// Readability-readerable.js — the cheap `isProbablyReaderable(doc)` gate.
    static let readerable = #"""
/*
 * Copyright (c) 2010 Arc90 Inc
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
