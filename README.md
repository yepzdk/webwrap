...
Next to it, the **list button** opens the reader's recents: the last 30 articles read in that app, newest first. Click a title to reopen it in the reader — even in an app where the reader isn't automatic. The list is per app and survives relaunch; **Clear history** at the bottom of the panel empties it (Restore Defaults, being an appearance reset, leaves it alone).
